...
class DrawerClose:
    ...
    def get_action(self):
        ...
        drawer_push_target_pos = (
            self.env.get_drawer_handle_pos() + np.array([0.04, 0.0, 0.1]))
        
        is_gripper_ready_to_push = (
            ee_pos[0] > drawer_push_target_pos[0] and
            np.linalg.norm(ee_pos[1] - drawer_push_target_pos[1]) < 0.1 and
            ee_pos[2] < drawer_push_target_pos[2] 
        )
        done = False
        neutral_action = [0.0]
        ...
        if (not self.env.is_drawer_closed() and
            ...
            action_xyz = (drawer_pos  - ee_pos) * self.xyz_action_scale
            # action_xyz[0] *= 3
            action_xyz[0] *= 1
...
#         self.drawer_never_opened = True
#         offset_coeff = (-1) ** (1 - self.env.left_opening)
#         self.handle_offset = np.array([offset_coeff * 0.2, 0.0, -0.01])
#         self.reached_pushing_region = False
#         self.neutral_taken = False
#         self.begin_closing = False
#         self.done = False
...
#         drawer_push_target_pos = (
#             drawer_pos + np.array([0.17, 0.0, 0.1]))
#         is_gripper_ready_to_push = (
#             ee_pos[0] > drawer_push_target_pos[0] and
#             ee_pos[2] < drawer_push_target_pos[2] 
#         )
